...
# Enable rclone copy to cloud storage
RCLONECOPY=0

# Remote cloud storage name
REMOTE="remote"

# Remote path
REMOTEPATH="backup/minecraft"
...
# Transfer files via rclone to cloud storage
if [ $RCLONECOPY -eq 1 ]
then
   echo "$(date +"%G-%m-%d %H:%M:%S") [LOG] Sending files to cloud storage via rclone"
   rclone copy $FINALDIR $REMOTE:$REMOTEPATH/$BACKUPDATE -v
fi
